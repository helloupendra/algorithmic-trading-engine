...
if [ -z "$ADMIN_USERNAME" ] || [ -z "$ADMIN_PASSWORD" ]; then
  die "ADMIN_USERNAME and ADMIN_PASSWORD must be set in .env to use the API."
fi
...
printf '    authenticating as admin ... '
TOKEN="$(curl -fsS -X POST "$API_BASE_URL/api/UserAuth/login" \
...
if [ -z "$TOKEN" ] || [ "$TOKEN" = "null" ]; then
  printf '\n'
  die "Failed to authenticate admin. Check ADMIN_PASSWORD in .env."
fi
printf 'done\n'
