...
def run_script(script_name, *args):
    ...
    try:
        # We use Popen for ingestor/monitor so they run and we don't block forever if we don't want to,
        # but for simplicity, we'll let subprocess.run block for the UI commands.
        # Wait, if we start ingestor, we probably want it to open in a new window or block the current one.
        # Since this is an interactive menu, starting the ingestor or monitor will block the menu.
        # Let's just use subprocess.run and they can Ctrl+C to get back to the menu!
        print("\n" + "="*50)
