...
class TitliMultiStraddleStrategy(BaseStrategy):
    name = "TitliMultiStraddle"

    def __init__(self, params: Optional[Dict[str, Any]] = None):
        self.params = params or {}
        self.adjustment_threshold = int(self.params.get("adjustment_threshold", 50))
    ...
    def on_bar(self, state: Dict[str, Any], inp: StrategyInput) -> List[StrategySignal]:
        ...
        if st1 < price:
            ...
            if st0 >= st1: st0 = 0
            if st2 <= st1: st2 = 0
            if st0 != 0 and max(st0 - price, price - st0) < 150:
                pe_value = st0
            else:
                st0 = 0
        else:
            ...
            if st0 >= st1: st0 = 0
            if st2 <= st1: st2 = 0
            if st2 != 0 and max(st2 - price, price - st2) < 150:
                ce_value = st2
            else:
                st2 = 0
        ...
        if legs_changed:
            if old_legs and old_group_id:
                ...
                signals.append(StrategySignal(
                    strategy_name=f"{self.name}{self.adjustment_threshold}",
            ...
            signals.append(StrategySignal(
                ...
                timestamp_utc=inp.timestamp_utc,
                reason=f"Titli Multi Straddle Adjusted. Active: {active_straddles}",
                price=price,
                legs=new_legs,
    ...
    def _group_id(self, timestamp_utc: str, counter: int) -> str:
        ...
        return f"TITLI-MULTI-{safe_ts}-{counter:03d}"