...
class FulcrumQtyAdjustmentStrategy(BaseStrategy):
    """Multi-straddle variant that doubles the short lots once three straddles are active."""
    name = "FulcrumQtyAdjustment"
    description = (
        "Same rolling multi-straddle as FulcrumMulti: one to three short straddles on the 100-point strikes "
        "around the spot plus far OTM call/put wings, rebuilt when the spot drifts past the adjustment "
        "threshold. When all three straddles are active the short legs are sold in double lots, so the "
        "position collects more premium in a tight range at the cost of a bigger loss on a breakout. Designed "
        "for BANKNIFTY-scale prices; needs live spot ticks and the full option chain."
    )
    category = "Adjustment"
    supported_underlyings = ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX"]
    legs_summary = "Sell 1-3 straddles around ATM (2x lots when 3 are active) + Buy far OTM CE and PE wings"
    default_lots = 1
    ...
    default_params: Dict[str, Any] = {"adjustment_steps": 0.7, "minor_steps": 0.1}
    ...
    def on_bar(self, state: Dict[str, Any], inp: StrategyInput) -> List[StrategySignal]:
        ...
        # A straddle within 1.7 strikes of the new level is kept rather than
        # closed and re-opened, which would pay the spread for nothing.
        keep_nearby = steps_to_points(1.7, step)
        ...
        short_lots = qty_multiplier * self.lots
        ...
        for st in active_straddles:
            new_legs.append({"symbol": f"{inp.underlying}_CE_{st}", "side": self.direction, "quantity": short_lots, "price": None})
            new_legs.append({"symbol": f"{inp.underlying}_PE_{st}", "side": self.direction, "quantity": short_lots, "price": None})
        ...
        if legs_changed:
            ...
            signals.append(StrategySignal(
                ...
                timestamp_utc=inp.timestamp_utc,
                reason=f"Fulcrum Qty Adjusted. Active: {active_straddles}, Qty: {qty_multiplier}",
                price=price,
                legs=new_legs,
    ...
    def _group_id(self, timestamp_utc: str, counter: int) -> str:
        ...
        return f"FULCRUM-QTY-{safe_ts}-{counter:03d}"